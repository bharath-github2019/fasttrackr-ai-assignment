...
const client = process.env.OPENAI_API_KEY
    ...
    : null;
const fallbackEnrichment = (transcript) => ({
    updates: {
    ...
    accountUpdates: [],
    bankUpdates: [],
    summary: "Audio processed without OpenAI key. Transcript captured in notes for manual review.",
    confidence: "low"
});
export const transcribeAndExtract = async (audioBuffer, fileName) => {
    if (!client) {
        return {
    ...
    const extraction = await client.responses.create({
        ...
        input: [
            {
                role: "system",
                content: "You are a financial CRM extraction assistant. Return strict JSON only. Extract possible household updates from transcript with conservative confidence."
            },
            {
                role: "user",
                content: transcript
